<?php
    ...
    class Donhangxuat{
        ...
        public static function Adddonhang_chitiet($madh, $masach, $slxuat) {
            ...
            // Kiểm tra xem đơn hàng chi tiết đã có mã đơn hàng xuất và mã sách chưa
            ...
            $ptkm = isset($ptkm) ? $ptkm : 0; // Nếu $ptkm là null, gán nó bằng 0
            ...
            if ($current_slxuat) {
                // Nếu tồn tại, cập nhật số lượng mới và thành tiền
                ...
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("issi", $new_slxuat, $new_thanhtien, $madh, $masach);
